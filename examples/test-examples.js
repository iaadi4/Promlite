  ...
  return new Promise((resolve) => {
    ...
    async function testServer() {
      try {
        ...
        // Test main endpoint
        const mainResponse = await makeRequest(`http://localhost:${example.port}/`);
        console.log(`   ✅ Main endpoint: ${mainResponse.status === 200 ? 'OK' : 'FAIL'}`);
        
        // Test metrics endpoint
        const metricsResponse = await makeRequest(`http://localhost:${example.port}/metrics`);
        const hasMetrics = metricsResponse.data.includes('# HELP') && metricsResponse.data.includes('# TYPE');
        ...
        if (example.name === 'counter') {
          ...
          const updatedMetrics = await makeRequest(`http://localhost:${example.port}/metrics`);
          const hasCounterMetric = updatedMetrics.data.includes('http_requests_total');
          console.log(`   ✅ Counter functionality: ${hasCounterMetric ? 'OK' : 'FAIL'}`);
        }
        
        if (example.name === 'comprehensive') {
          await makeRequest(`http://localhost:${example.port}/api/users`);
          const updatedMetrics = await makeRequest(`http://localhost:${example.port}/metrics`);
          const hasAllMetrics = updatedMetrics.data.includes('http_requests_total') && 
                               updatedMetrics.data.includes('active_connections') && 
                               updatedMetrics.data.includes('http_request_duration_seconds');
          console.log(`   ✅ All metric types: ${hasAllMetrics ? 'OK' : 'FAIL'}`);
        }
    ...
    setTimeout(() => {
      if (!started) {
        console.log(`   ❌ ${example.name} server failed to start within 10 seconds`);
        server.kill();
        resolve();
      }
    }, 10000);
...
async function main() {
  ...
  try {
    ...
    console.log('❌ Built distribution not found. Run "npm run build" in the parent directory first.');
    process.exit(1);
  }
